...
async function loadStatistics() {
    try {
        ...
        if (ordersRes.ok) {
            ...
            // Mai dátum (helyi időzóna szerint, YYYY-MM-DD formátum)
            const now = new Date();
            ...
            orders.forEach(o => {
                ...
                // Mai rendelések ellenőrzése
                if (o.created_at) {
                    let orderDate = '';
                    if (typeof o.created_at === 'string') {
                        orderDate = o.created_at.split('T')[0].split(' ')[0];
                    } else if (o.created_at instanceof Date) {
                        orderDate = o.created_at.toISOString().split('T')[0];
                    }
...
async function deleteDish(id, name) {
    ...
    if (!confirm(`Biztosan törölni szeretnéd a(z) "${name}" ételt?\n\nEz a művelet nem vonható vissza!`)) {
        return;
    }
